...
function start(): void {
  ...
  alertaEstoqueWorker.on('completed', (job) => {
    logger.info({ jobId: job.id }, 'Job da fila alerta-estoque concluido');
  });

  alertaEstoqueWorker.on('failed', (job, err) => {
    logger.error({ jobId: job?.id, err }, 'Job da fila alerta-estoque falhou');
  });
